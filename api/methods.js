...
module.exports = {
	...
	printPrices: function(orders) {
		...
		orders.forEach(order => {
			...
			orderJson.Order_ID = order.order_number;
			orderJson.Order_Items = [];
			let total = 0;
			...
			order.order_items.forEach(item => {
				...
				orderJson.Order_Items.push(`Order item: ${item.type}: $${price}`);
			});
			// print total for order
			console.log(`\tOrder total: $${total}`);
			console.log(`\n`);
			orderJson.Order_Items.push(`Order total: $${total}`);
			json.push(orderJson);
		});
  	...
  	let json = [];
		...
		orders.forEach(order => {
			...
			for (let fund of Object.keys(orderTotal)) {
					...
					orderJson.Fund_Distributions.push(`Fund - ${fund}: $${orderTotal[fund]}`);
			}
		...
		for (let fund of Object.keys(totals)) {
			...
			totalJson.Total_Distributions.push(`Fund - ${fund}: $${totals[fund]}`);
		}
